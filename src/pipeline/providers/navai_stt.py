"""
NavAI STT (Speech-to-Text) provider.
Uses NavAI GPU server for Uzbek speech recognition.
Endpoint: POST /stt/transcribe/transcribe-batch
"""
...
class NavaiSTT(stt.STT):
    """NavAI custom STT for Uzbek language."""
    ...
    def __init__(
        ...
        *,
        ...
        # Host is env-driven (no hardcoded default). Set NAVAI_STT_URL to your
        # GPU STT server, or use a managed provider instead of this one.
        self._base_url = (base_url or os.getenv("NAVAI_STT_URL", "")).rstrip("/")
        self._endpoint = f"{self._base_url}/stt/transcribe/transcribe-batch"
        ...
        logger.info(f"NavAI STT initialized: url={self._base_url}, lang={self._language}")
        ...
        *,
        ...
        try:
            ...
            if transcript and transcript.strip():
                logger.info(
                    f"NavAI STT [{audio_duration:.2f}s -> {total_ms:.0f}ms]: "
                    f"'{transcript[:50]}'"
                )
    ...
    async def _transcribe(self, wav_data: bytes, language: Optional[str] = None) -> str:
        ...
        try:
            ...
            form.add_field("languages", lang)

            async with session.post(self._endpoint, data=form) as response:
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"NavAI STT API error {response.status}: {error_text[:200]}")
                    return ""
                ...
                if isinstance(result, list) and len(result) > 0:
                    ...
                    return result.get("text", result.get("result", "")).strip()

                return ""

        except aiohttp.ClientError as e:
            logger.error(f"NavAI STT network error: {e}")
            return ""
        except Exception as e:
            logger.error(f"NavAI STT error: {e}")
            return ""
    ...
    def _prepare_wav(self, buffer: Union[rtc.AudioFrame, bytes]) -> bytes:
        ...
        try:
            ...
            logger.error(f"NavAI STT audio prep error: {e}")
            return b""
